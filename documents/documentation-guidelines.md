...
* ただし、Unicode に定義されている文字であっても以下のいずれかに該当しないものは可能な限り使用しない(SHOULD)。
...
* 数字、記号は可能な限り[JIS X 0201 ラテン文字用図形文字集合](http://ja.wikipedia.org/wiki/JIS_X_0201#.E3.83.A9.E3.83.86.E3.83.B3.E6.96.87.E5.AD.97.E7.94.A8.E5.9B.B3.E5.BD.A2.E6.96.87.E5.AD.97.E9.9B.86.E5.90.88)のものを用いる。
* [約物](http://ja.wikipedia.org/wiki/%E7%B4%84%E7%89%A9)は全角記号を使用する。句点として "。"、読点として"、"を使用する。
...
ただし、以下に該当する語については常用漢字によって記載可能であってもかな書きとする。
...
『[現代仮名遣い(内閣告示第一号)](http://www.mext.go.jp/b_menu/hakusho/nc/k19860701001/k19860701001.html)』 に従う(MUST)。

# 送り仮名

『[送り仮名の付け方(内閣告示第二号)](http://www.mext.go.jp/b_menu/hakusho/nc/k19730618001/k19730618001.html)』に従う(MUST)。
...
『[外来語の表記(内閣告示第二号)](http://www.mext.go.jp/b_menu/hakusho/nc/k19910628002/k19910628002.html)』に従う(MUST)。

## 『外来語の表記(内閣告示第二号)』(転載、部分抜粋)
...
ただし、一般的な技術用語で慣習的にカタカナ表記が用いられる語が、その文脈において使用される場合は除く。

```
〔例〕クラス　インターフェース　メソッド　フィールド　プロパティー　オブジェクト　インスタンス
```

* 特定の技術要素(パッケージ名、モジュール名、クラス名、ファイル名、タグ名、属性名など)を表す語
...
* 慣習的にカタカナ表記、英語表記のいずれかを用いる語については慣習に従う。
